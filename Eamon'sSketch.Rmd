...
data_CA <- data_CCA %>%
  select("Mode.of.reproduction", "Prefered.Habitat.Type")
freq_CA <- table(data_CCA$Mode.of.reproduction, data_CCA$Prefered.Habitat.Type)

# Make a frequency table containing all 4 categorical variables to be used
freq_CCA <- table(data_CCA$Foraging.Mode, data_CCA$Distribution, data_CCA$Mode.of.reproduction, data_CCA$Prefered.Habitat.Type)
```

### Represent Data as a Heatmap
```{r}
freq_CA %>%  
...
### Make a Tibble
```{r}
# Convert to DF as the function wont accept the table
freq_CA_df <- as.data.frame(freq_CA) %>%
  # Rename the variables as they lost their names while converting to DF
  rename(Mode.of.reproduction = `Var1`,  Prefered.Habitat.Type = `Var2`) %>%
  # Convert factors to character as function wont accept factors
  mutate(across(where(is.factor), as.character))
  

freq_CA_long <- freq_CA_df %>%
  rownames_to_column(var = "Reproduction") %>%
  pivot_longer(
    cols = -c("Mode.of.reproduction"),  # All columns except "Mode.of.reproduction"
    names_to = "Prefered.Habitat.Type",  # Column names become "Prefered.Habitat.Type"
    values_to = "NbIndividuals"          # Values go into "NbIndividuals"
  )
...
freq_CA_long <- freq_CA_df %>%
  pivot_longer(
    cols = -c("Mode.of.reproduction"),  # Keep Mode.of.reproduction
    names_to = "Prefered.Habitat.Type",
    values_to = "NbIndividuals"
  )

freq_CA_long <- as.data.frame(freq_CA) %>%  # Convert to data frame
  mutate(across(where(is.factor), as.character)) %>%
  rownames_to_column(var = "Reproduction") %>%  # Create a column for row names
  pivot_longer(
    cols = -c("Mode.of.reproduction"),  # All columns except "Mode.of.reproduction"
    names_to = "Prefered.Habitat.Type",  # Column names become "Prefered.Habitat.Type"
    values_to = "NbIndividuals"          # Values go into "NbIndividuals"
