...
freq_CA_short <- data_CCA %>%
  select(Prefered.Habitat.Type, Mode.of.reproduction) %>%
  count(Prefered.Habitat.Type, Mode.of.reproduction) %>%  
  pivot_wider(
    names_from = Mode.of.reproduction, 
    values_from = n,  
    values_fill = 0)
...
### Represent in ggplot
```{r}
ggplot(freq_CA_tibble) +
  aes(x = Reproduction, y = Habitat, fill = Freq) + # fill says which column is used 
  # for filling
